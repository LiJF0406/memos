...
-- Create the default folder for each user (idempotent).
INSERT INTO note_folder (uid, creator_id, parent_id, name, shared, is_default)
SELECT 'inbox-' || id, id, NULL, 'My Notes', FALSE, TRUE
FROM "user"
WHERE NOT EXISTS (
  SELECT 1 FROM note_folder WHERE creator_id = "user".id AND is_default = TRUE
);
...
SET parent_id = (
  SELECT dnf.id FROM note_folder AS dnf WHERE dnf.creator_id = nf.creator_id AND dnf.is_default = TRUE
)
