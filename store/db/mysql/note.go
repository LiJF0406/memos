...
func (d *DB) CreateNote(ctx context.Context, create *store.Note) (*store.Note, error) {
	...
	if create.CreatedTs != 0 {
		...
		placeholders = append(placeholders, "FROM_UNIXTIME(?)")
	...
	stmt := "INSERT INTO `note` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholders, ", ") + ")"
	...
	id := int32(rawID)
	notes, err := d.ListNotes(ctx, &store.FindNote{ID: &id})
	if err != nil {
		return nil, err
	}
	if len(notes) == 0 {
		return nil, errors.Errorf("failed to create note")
	}
	return notes[0], nil
}
...
func buildNoteQuery(find *store.FindNote) (join []string, joinArgs []any, where []string, args []any) {
	...
	if v := find.CreatedTsAfter; v != nil {
		where, args = append(where, "UNIX_TIMESTAMP(`note`.`created_ts`) >= ?"), append(args, *v)
	}
...
func (d *DB) ListNotes(ctx context.Context, find *store.FindNote) ([]*store.Note, error) {
	...
	fields := []string{
		...
		"UNIX_TIMESTAMP(`note`.`created_ts`) AS `created_ts`",
		"UNIX_TIMESTAMP(`note`.`updated_ts`) AS `updated_ts`",
	}
...
func (d *DB) ListNoteCreatedTs(ctx context.Context, find *store.FindNote) ([]int64, error) {
	...
	query := "SELECT UNIX_TIMESTAMP(`note`.`created_ts`) FROM `note` " +
		strings.Join(join, " ") +
		" WHERE " + strings.Join(where, " AND ") +
