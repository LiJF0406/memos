...
func (d *DB) CreateNoteFolder(ctx context.Context, create *store.NoteFolder) (*store.NoteFolder, error) {
	...
	stmt := "INSERT INTO `note_folder` (" + strings.Join(fields, ", ") + ") VALUES (" + strings.Join(placeholders, ", ") + ")"
	...
	id := int32(rawID)
	folders, err := d.ListNoteFolders(ctx, &store.FindNoteFolder{ID: &id})
	if err != nil {
		return nil, err
	}
	if len(folders) == 0 {
		return nil, errors.Errorf("failed to create note folder")
	}
